...
We can use the `predict()` function to predict outcomes. Note that the argument `type = "class"` instructs `R` to return the actual class prediction.
...
We correctly classify about 78.9% of the observations in the test data set.
...
Next, we use cost-complexity pruning to see if we can simplify the tree and decrease variance without increasing bias too much. The function `cv.tree()` performs cross-validation in order to determine the optimal level of tree complexity; cost complexity pruning is used in order to select a sequence of trees for consideration. We then plot the cross-validation error against the number of terminal nodes of each tree (`size`) as well as the value of the cost-complexity parameter (`k`, which corresponds to $\alpha$ in (2.1.2) in the slides.)
...
Again, 78.9% of the test observations are correctly classified, so the pruning process has not improved classification accuracy.
...
The tree with 3 terminal nodes has the lowest cross-validation error. We apply the `prune.misclass()` function in order to prune the tree to obtain the 3-node tree.
...
We slightly decreased the MSE by about 1.6%.

```{r, echo=TRUE}
(8.769241 - 8.63068) / 8.769241
```
