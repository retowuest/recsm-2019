...
We now perform 5-fold cross-validation, plot the CV estimate of the expected test MSE for each lambda, and use the test set to compute the test error for the model with the optimal $\lambda$.
...
We see that the lasso further improves prediction accuracy compared to ridge regression with $\lambda$ chosen by cross-validation. In addition, the lasso has the advantage that it sets some coefficient estimates exactly to 0, thus producing a more interpretable model than ridge regression.
